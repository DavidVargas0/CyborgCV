...
class PenDetectionDebugger:
    ...
    def detect_white_canvas(self, frame, show_debug=True):
        ...
        if show_debug:
            cv2.imshow('2. White Threshold (>200)', self.resize_for_display(thresh))
        ...
        if show_debug:
            ...
            cv2.imshow('3. All White Contours', self.resize_for_display(contour_img))
        ...
        if contours:
            ...
            if show_debug:
                ...
                cv2.imshow('4. Top 3 Largest Areas', self.resize_for_display(top3_img))
    ...
    def detect_yellow_pen(self, frame, canvas_bounds, show_debug=True):
        ...
        if show_debug:
            cv2.imshow('5. Canvas Region (BGR)', self.resize_for_display(canvas_region))
            cv2.imshow('6. Canvas Region (HSV)', self.resize_for_display(hsv))
        ...
        for name, lower, upper in yellow_ranges:
            ...
            if show_debug:
                ...
                cv2.imshow(f'7. {name} Mask', self.resize_for_display(mask_display))
        ...
        if show_debug:
            cv2.imshow('8. Cleaned Yellow Mask', self.resize_for_display(yellow_mask_clean))
        ...
        if show_debug:
            ...
            cv2.imshow('9. Yellow Contours', self.resize_for_display(yellow_contours_img))
    ...
    def resize_for_display(self, img, max_width=800):
    ...
    def debug_first_frame(self):
        ...
        # Show original
        cv2.imshow('0. Original Frame', self.resize_for_display(frame))

        # Detect canvas
        print("\n--- STEP 1: CANVAS DETECTION ---")
        canvas_bounds = self.detect_white_canvas(frame, show_debug=True)

        # Detect pen
        print("\n--- STEP 2: PEN DETECTION ---")
        pen_pos = self.detect_yellow_pen(frame, canvas_bounds, show_debug=True)
        ...
        cv2.imshow('10. FINAL RESULT', self.resize_for_display(result_img))
